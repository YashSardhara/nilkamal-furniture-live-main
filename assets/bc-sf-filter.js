...
BCSfFilter.prototype.buildProductGridItem = function (data, index) {
    ...
    itemProductInfoHtml += '{{percentageOff}}';
    ...
    // Add Price
    var itemPriceHtml = '';
  	var customPriceMin = data.price_min;
 	var customPriceCompared = data.compare_at_price_max;
    if (typeof comingsoonLabel[0] !== 'undefined') {
        ...
        if (data.price_min > 0) {
          if (data.tags.includes('Best Deals') || bcSfFilterConfig.general.collection_handle === 'best-deals-1'){
            itemPriceHtml += '<span class="money pfs-special-price">₹' + this.formatMoney(customPriceMin * 0.9) + '</span>';
            itemPriceHtml += '<span class="money pfs-cross-regular-price">₹' + this.formatMoney(customPriceMin) + '</span>';
          } else {
            itemPriceHtml += '<span class="money">₹' + this.formatMoney(customPriceMin) + '</span>';
          }
        } else {
        ...
        if (onSale) {
            if (priceVaries && data.price_min > 0) {
                var variant = data.variants.find(function(e){return e.price == customPriceMin});
                if (typeof variant != 'undefined') {
                    customPriceCompared = variant.compare_at_price;
                }
              if (data.tags.includes('Best Deals') || bcSfFilterConfig.general.collection_handle === 'best-deals-1'){
                itemPriceHtml += ' <span class="was_price pfs-margin-none"><span class="money">₹' + this.formatMoney(customPriceCompared) + '</span></span>';
              } else {
    ...
    //Add discount percentage off label
    var percentageOff = '';
    if (onSale) {
        percentageOff += '<span class="savings">' + data.percent_sale_min + '% off</span>';
    }
    itemHtml = itemHtml.replace(/{{percentageOff}}/g, percentageOff);
            ...
            if (colorTypes.indexOf(downcasedOption) > -1) {
                ...
                for (var i in data.variants) {
                    ...
                    if (values.indexOf(value) == -1) {
                        ...
                        var fileColorUrl = this.optimizeImage(bcSfFilterConfig.general.no_image_url, '50x');
    ...
    // Add wishlist
    var itemWishListHtml = '<a class="iWishAddColl iwishcheck" data-variant="' + firstVariant.id + '" data-product="{{itemId}}" data-ptitle="{{itemTitle}}">';
    itemWishListHtml += '<!--?xml version="1.0" ?--><svg fill="none" class="icon" height="24" width="24" stroke="#000" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg>';
    itemWishListHtml += '  </a>';
...
BCSfFilter.prototype.buildAdditionalElements = function (data) {
    ...
    if (data.total_product == 1) {
      var productCount = `(${data.total_product} Product)`;
    } else {
      var productCount = `(${data.total_product} Products)`;
    }
    jQ('.bc-sf-filter-product-count').html(productCount);
